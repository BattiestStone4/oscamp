use super::SET_SIZE;
use allocator::{AllocError, ByteAllocator};
...
impl<const BLK_SIZE: usize> Slab<BLK_SIZE> {
    ...
    pub fn allocate(
        &mut self,
        _layout: Layout,
        buddy: &mut tlsf::TlsfByteAllocator,
    ) -> allocator::AllocResult<NonNull<u8>> {
        match self.free_block_list.pop() {
            Some(block) => Ok(NonNull::new(block.addr() as *mut u8).unwrap()),
            None => {
                let layout =
                ...
                if let Ok(ptr) = buddy.alloc(layout) {
                    ...
                    Ok(NonNull::new(self.free_block_list.pop().unwrap().addr() as *mut u8).unwrap())
                } else {
